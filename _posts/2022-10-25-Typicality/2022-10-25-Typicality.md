...
**Lemma 5:**
	(implimentations of RV convergences):<br>
...
or equivalently, let $$\mathcal{W}_{[X]{\epsilon}}^n\stackrel{\Delta}{=}\{x^n\in \mathcal{X}^n: \vert{}-\frac{\log p(x^n)}{n}-H(X)\vert{}\leq \epsilon\}$$, then
...
* $$\forall \epsilon>0,\exists N\in\mathbb{N}_+,\forall n>N, (1-\epsilon)2^{n(H(X)-\epsilon)}\leq \vert{}\mathcal{W}_{[X]\epsilon}^n\vert{}\leq 2^{n(H(X)+\epsilon)}.$$
...
The weak AEP shows that a small set (compared to $$\mathcal{X}^n$$, $$\mathcal{W}_{[X]\epsilon}^n$$ is exponentially small) contains almost all probability of $$\mathcal{X}^n$$ with nearly uniform distribution. So randomly choose a sequence from $$\mathcal{X}^n$$, the probability are near $$\frac{1}{\vert{}\mathcal{W}_{[X]\epsilon}^n\vert{}}$$ with high probability.* Note: The weekly typical sequence is different from the most likely sequence. It is not necessary fora a weakly typical set to include most likely sequences to possess the probability 1.
...
which is a ploynomial of n* Note: The question of the number of types is equivalent to the question, in which n different balls are independently labeled with $$\vert{}\mathcal{X}\vert{}$$ labels, or we choose n unordered balls from an urn containing $$\vert{}\mathcal{X}\vert{}$$balls with replacement.
...
It is worth noticing that $$\frac{1}{n}log\vert{}\mathcal{T}_Q^n\vert{}\sim H(Q)$$is a exponential approximation, which allows an deviation of a ploynomial times of the size.* Note: This is true that for any ploynomial $$P(x)\in \mathbb{R}[x]$$, $$\lim\limits_{n\to \infty}\dfrac{e^x}{P(x)}=0$$.
...
([here](#relative-entropy)) shows that the difference between $$H(Q)$$and $$H(Q,P_X)$$(i.e., the difference of empirical distribution and real distribution) determines the probability of a certain type. (Compared to the size of a type, which is only determined by the empirical distribution). We can intuitively cosnsider $$D(Q\vert{}\vert{}P_X)$$as the "distance" between 2 distributions. The large the distance of $$Q$$and $$P_X$$is, the less $$P_{X^n}(\mathcal{T}_Q^n)$$will be. As a specific case when $$P=Q$$, $$D(Q\vert{}\vert{}P_X)=0$$and the probability gets maximum.
