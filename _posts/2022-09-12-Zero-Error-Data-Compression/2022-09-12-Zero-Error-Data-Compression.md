...
**Definition 1.1 (Source Code).** A $$D$$-ary source code for an i.i.d. sequence $$X_{[n]}$$ is a mapping $$\mathcal C:\mathcal X^n\to[0:D-1]^{<\infty}$$. We call a mapping $$\mathcal C^*:\mathcal X^{n\times<\infty}\to[0:D-1]^{<\infty}$$ the extension code of $$\mathcal C$$ iff
...
**Example.** Let $$\mathcal X=\{a,b\}$$, $$n=2$$, $$D=2$$,
...
**Lemma 1.1.** All prefix codes are uniquely decodable.

**Proof.** We can construct a decoding process by searching the code tree of a prefix code until the leaves and decode, which is unique and correct.

We are naturally interested in the expected length of a codeword.

**Definition 1.2 (Rate of 0-error Variant Data Compression).** Let $$X$$ be a finite random source with $$\lvert\mathcal X\rvert=m\geq2$$ and $$\mathcal C$$ be a uniquely decodable code. The expected length of $$\mathcal C$$ is given by
...
**Theorem 1.2 (Capacity of 0-error Data Compression).** The capacity of the 0-error data compression problem with RV $$X$$ is $$H(X)$$.

We give the proof in two parts, achievability and converse.

## 1.2 Proof of Theorem 1.2: Converse

The converse claims that no UDC $$\mathcal C$$ can achieve its expected length less than $$H(X)$$.

**Theorem 1.3 (Kraft Inequality).** Let $$X$$ be an RV with $$\lvert\mathcal X\rvert\geq2$$, let $$\mathcal C$$ be a $$D$$-ary UDC, and let $$l_k=l(\mathcal C(x_k))$$. Then
...
In the next theorem, we prove the converse of Theorem 1.2.

**Theorem 1.4 (Converse of 0-error Data Compression).** All UDC $$\mathcal C$$ is lower bounded by $$H_D(X)$$, i.e.,
...
**Lemma 1.5.** $$H(X)\leq\log\lvert\mathcal X\rvert$$.

**Proof.** Consider a $$\lvert\mathcal X\rvert$$-ary code for RV $$X$$. Obviously there exists a UDC with $$L=1$$. Then we can derive from Theorem 1.4 that
...
**Definition 1.3 (Redundancy of UDC).** The redundancy of a UDC $$\mathcal C$$ is defined by
...
## 1.3 Proof of Theorem 1.2: Achievability
...
**Theorem 1.6 (Existence of prefix codes).** Let $$X$$ be an RV. There exists a $$D$$-ary prefix code with codeword lengths $$l_1,\ldots,l_m$$ iff the Kraft inequality is satisfied.

**Proof.** We only need to prove the “if” part. W.l.o.g., consider $$l_1\leq\cdots\leq l_m$$. Consider a full $$D$$-ary tree with depth $$l_m$$. We construct a prefix code $$\mathcal C$$ by the following steps, called pruned trees:
...
**Example.** $$D=2$$ and $$l_{[5]}=(1,2,3,4,4)$$. We know the lengths satisfy Kraft's inequality. A possible pruned tree is:
...
**Lemma 1.7.** There exists a $$D$$-ary prefix code $$\mathcal C$$ such that $$\mathbb E[l(\mathcal C(X))]=H(X)$$ iff $$\{p_i\}_{i\in[m]}$$ are all $$D$$-adic.

**Proof.** The “if” part is a derivation of construction from Theorem 1.6, and the “only if” part is from the equality condition of Theorem 1.4.
...
**Definition 1.4 (Huffman Code).** Let $$X:\Omega\to\mathcal X$$ be an RV and $$D\geq2$$ be a positive integer. We construct an RV $$X'$$ with $$\mathcal X\subseteq\mathcal X'$$ such that
...
**Lemma 1.8.** Optimal code maps larger probabilities to shorter codewords.

**Proof.** Suppose there are two probability masses $$p_i<p_j$$ with optimal code lengths $$c_i<c_j$$. We can exchange the two codewords to get a new prefix code, but
...
**Lemma 1.9.** There exists an optimal code $$\mathcal C$$ in which the two smallest probabilities are siblings (have the same length and differ only in the last symbol).

**Proof.** If not, we can code the smallest probability to its parent (note that it is longest and has no sibling) and reduce the length of the codeword by $$1$$, which contradicts the optimality.

We further prove that the Huffman code is actually the optimal code.

**Theorem 1.10.** Huffman code is optimal.

**Proof.** Suppose an optimal code $$\mathcal C:\mathcal X\to[2]^{<\infty}$$ and let $$\{p_i\}$$ be the probabilities in descent order. For siblings $$p_{m-1}$$ and $$p_m$$, we merge them together and code the new probability by its parent. Then we get another prefix code $$\mathcal C^*$$ with length
...
**Theorem 1.11 (Infimum of Huffman Code).** All expected lengths of Huffman code have an infimum:
...
**Proof.** Let $$X$$ be an RV. Notice that Huffman code is the optimal code. As long as we construct a prefix code $$\mathcal C$$ with expected length less than $$H(X)-1$$, we can prove $$L(\mathcal H)<H(X)+1$$. Consider a $$D$$-ary prefix code with codeword length $$l_i=\lceil-\log_Dp_i\rceil$$. The existence of such a code $$\mathcal C$$ is proved in Theorem 1.6, because $$\{l_i\}$$ satisfies the Kraft inequality:
...
**Theorem 1.12 (Achievability of 0-error Data Compression II: By Huffman Code).** For any RV $$X$$, Huffman code gives the entropy bound in the average sense, i.e.,
...
**Proof.** This is proved by the definition of prefix code (that the expanded code of Huffman code is also a Huffman code), and that
...
**Lemma 1.14 (Performance of mismatched).**
...
**Definition 1.6 (Related Definitions of Coding Tree).** Let $$X\sim\{p_1,\ldots,p_m\}$$ be an RV and $$\mathcal C$$ be a $$D$$-ary prefix code. Let $$G=(V,E)$$ be the corresponding coding tree. Let $$I$$ be the index set of all the internal nodes (including the root) in $$G$$.
...
**Lemma 1.15 (Basic Results in the Coding Tree).**
...
The theorem of entropy bound (Theorem 1.2) is then expressed by:

**Theorem 1.16 (Capacity of 0-error Data Compression).** The redundancy of any prefix code $$R=L-H(X)\geq0$$, with equality iff all the internal nodes are balanced.
